...
로컬 Claude Code 설정을 읽어서 이 프로젝트의 registry 디렉토리에 있는 JSON 파일들을 초기화하고, init.sh를 실행합니다.

## 수행 작업

1. **MCP 서버 설정 읽기**: `~/.claude.json` 파일에서 MCP 서버 정보를 읽어 `registry/mcp-servers.json`에 저장
2. **Marketplace 설정 읽기**: `claude plugin marketplace list` 명령으로 marketplace 목록을 읽어 `registry/marketplaces.json`에 저장
3. **Plugin 설정 읽기**: `claude plugin list` 명령으로 설치된 플러그인 목록을 읽어 `registry/plugins.json`에 저장
4. **init.sh 실행**: registry 파일 생성 후 `./init.sh`를 실행하여 설정 적용
...
### Step 4: init.sh 실행
1. 위 단계가 모두 완료되면 스킬 디렉토리의 `./init.sh`를 실행합니다.
