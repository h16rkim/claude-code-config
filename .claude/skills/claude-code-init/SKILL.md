...
로컬 Claude Code 설정을 읽어서 이 프로젝트에 백업합니다. config 디렉토리와 registry JSON 파일들을 동기화한 후, init.sh를 실행합니다.

## 수행 작업

1. **Config Symlink 생성**: 프로젝트의 `config/` 디렉토리 내 모든 항목(파일 및 디렉토리)을 `~/.claude/` 디렉토리에 symlink로 연결
2. **MCP 서버 설정 읽기**: `~/.claude.json` 파일에서 MCP 서버 정보를 읽어 `registry/mcp-servers.json`에 저장
3. **Marketplace 설정 읽기**: `claude plugin marketplace list` 명령으로 marketplace 목록을 읽어 `registry/marketplaces.json`에 저장
4. **Plugin 설정 읽기**: `claude plugin list` 명령으로 설치된 플러그인 목록을 읽어 `registry/plugins.json`에 저장
5. **init.sh 실행**: 위 작업 완료 후 `init.sh`를 실행하여 설정 적용
...
### Step 1: Config Symlink 생성
1. `~/.claude` 디렉토리가 존재하는지 확인하고, 없으면 생성합니다.
2. 프로젝트의 `config/` 디렉토리 내 모든 항목(파일 및 디렉토리)을 순회합니다.
3. 각 항목에 대해 `~/.claude/{항목명}` symlink를 생성하여 프로젝트의 실제 파일/디렉토리를 가리키도록 합니다.
4. 기존 파일, 디렉토리 또는 symlink가 있으면 백업 후 새로 생성합니다.
5. 예:
